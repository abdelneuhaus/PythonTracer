...
import python_tracer.decode.neuralfitter.coord_transform
import python_tracer.decode.neuralfitter.utils
import python_tracer.decode.simulation
import python_tracer.decode.utils
from python_tracer.decode.neuralfitter.train.random_simulation import setup_random_simulation
from python_tracer.decode.neuralfitter.utils import log_train_val_progress
from python_tracer.decode.utils.checkpoint import CheckPoint
...
def live_engine_setup(param_file: str, device_overwrite: str = None, debug: bool = False,
    ...
    param = python_tracer.decode.utils.param_io.ParamHandling().load_params(param_file)

    # auto-set some parameters (will be stored in the backup copy)
    param = python_tracer.decode.utils.param_io.autoset_scaling(param)

    # add meta information
    param.Meta.version = python_tracer.decode.utils.bookkeeping.decode_state()
    ...
    python_tracer.decode.utils.param_io.ParamHandling().write_params(param_backup, param)

    if debug:
        python_tracer.decode.utils.param_io.ParamHandling.convert_param_debug(param)
    ...
    if torch.cuda.is_available():
        _, device_ix = python_tracer.decode.utils.hardware._specific_device_by_str(device)
    ...
    """Setup Log System"""
    if no_log:
        logger = python_tracer.decode.neuralfitter.utils.logger.NoLog()

    else:
        log_folder = log_folder + '/' + experiment_id

        logger = python_tracer.decode.neuralfitter.utils.logger.MultiLogger(
            [python_tracer.decode.neuralfitter.utils.logger.SummaryWriter(log_dir=log_folder,
                                                            filter_keys=["dx_red_mu", "dx_red_sig",
                                                                         ...
                                                                         "dphot_red_mu",
                                                                         "dphot_red_sig"]),
             python_tracer.decode.neuralfitter.utils.logger.DictLogger()])
    ...
    if from_ckpt:
        ckpt = python_tracer.decode.utils.checkpoint.CheckPoint.load(param.InOut.checkpoint_init)
    ...
    while not converges and n < n_max:
        ...
        conv_check = python_tracer.decode.neuralfitter.utils.progress.GMMHeuristicCheck(
            ref_epoch=1,
        ...
        for i in range(first_epoch, param.HyperParameter.epochs):
            ...
            if i >= 1:
                _ = python_tracer.decode.neuralfitter.train_val_impl.train(
                    model=model,
                    optimizer=optimizer,
                    loss=criterion,
                    dataloader=dl_train,
            ...
            val_loss, test_out = python_tracer.decode.neuralfitter.train_val_impl.test(
                model=model,
                loss=criterion,
                dataloader=dl_test,
                epoch=i,
...
def setup_trainer(simulator_train, simulator_test, logger, model_out, ckpt_path, device, param):
    ...
    models_available = {
        'SigmaMUNet': python_tracer.decode.neuralfitter.models.SigmaMUNet,
        'DoubleMUnet': python_tracer.decode.neuralfitter.models.model_param.DoubleMUnet,
        'SimpleSMLMNet': python_tracer.decode.neuralfitter.models.model_param.SimpleSMLMNet,
    }

    model = models_available[param.HyperParameter.architecture]
    model = model.parse(param)

    model_ls = python_tracer.decode.utils.model_io.LoadSaveModel(model,
    ...
    """Loss function."""
    criterion = python_tracer.decode.neuralfitter.loss.GaussianMMLoss(
    ...
    frame_proc = python_tracer.decode.neuralfitter.scale_transform.AmplitudeRescale.parse(param)
    bg_frame_proc = None

    if param.HyperParameter.emitter_label_photon_min is not None:
        em_filter = python_tracer.decode.neuralfitter.em_filter.PhotonFilter(
            param.HyperParameter.emitter_label_photon_min)
    else:
        em_filter = python_tracer.decode.neuralfitter.em_filter.NoEmitterFilter()
    ...
    tar_gen = python_tracer.decode.neuralfitter.utils.processing.TransformSequence(
        [
            python_tracer.decode.neuralfitter.target_generator.ParameterListTarget(
            ...
            python_tracer.decode.neuralfitter.target_generator.DisableAttributes.parse(param),

            python_tracer.decode.neuralfitter.scale_transform.ParameterListRescale(
    ...
    if param.Simulation.mode == 'acquisition':
        train_ds = python_tracer.decode.neuralfitter.dataset.SMLMLiveDataset(
        ...
        train_ds = python_tracer.decode.neuralfitter.dataset.SMLMLiveSampleDataset(
            simulator=simulator_train,
            em_proc=em_filter,
            frame_proc=frame_proc,
            bg_frame_proc=bg_frame_proc,
    ...
    test_ds = python_tracer.decode.neuralfitter.dataset.SMLMAPrioriDataset(
    ...
    if param.PostProcessing is None:
        post_processor = python_tracer.decode.neuralfitter.post_processing.NoPostProcessing(xy_unit='px',
                                                                              px_size=param.Camera.px_size)

    elif param.PostProcessing == 'LookUp':
        post_processor = python_tracer.decode.neuralfitter.utils.processing.TransformSequence([

            python_tracer.decode.neuralfitter.scale_transform.InverseParamListRescale(
                phot_max=param.Scaling.phot_max,
                z_max=param.Scaling.z_max,
                bg_max=param.Scaling.bg_max),

            python_tracer.decode.neuralfitter.coord_transform.Offset2Coordinate.parse(param),

            python_tracer.decode.neuralfitter.post_processing.LookUpPostProcessing(
        ...
        post_processor = python_tracer.decode.neuralfitter.utils.processing.TransformSequence([
            ...
            python_tracer.decode.neuralfitter.post_processing.SpatialIntegration(
                raw_th=param.PostProcessingParam.raw_th,
                xy_unit='px',
                px_size=param.Camera.px_size)
        ])
    ...
    matcher = python_tracer.decode.evaluation.match_emittersets.GreedyHungarianMatching.parse(param)
...
def setup_dataloader(param, train_ds, test_ds=None):
    ...
    train_dl = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        collate_fn=python_tracer.decode.neuralfitter.utils.dataloader_customs.smlm_collate)
