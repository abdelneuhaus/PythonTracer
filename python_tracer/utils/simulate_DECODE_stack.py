import python_tracer.decode
import python_tracer.decode.utils as utils
import python_tracer.decode.simulation as simulation
import python_tracer.decode.neuralfitter.train.live_engine as train_live_engine
...
def simulate_DECODE_stack(calib_file, 
    ...
    utils.param_io.copy_reference_param('')  # saved in current dir
    param = utils.param_io.load_params('./param_friendly.yaml')  # change path if you load custom file
    ...
    simulator, sim_test = train_live_engine.setup_random_simulation(param)
    camera = simulation.camera.Photon2Camera.parse(param)

    param = utils.param_io.autoset_scaling(param)
